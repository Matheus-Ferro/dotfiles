...
local chrome            = "/opt/google/chrome/chrome -no-default-browser-check"
...
awful.util.tagnames = {  "Ⅰ", "Ⅱ", "Ⅲ", "Ⅳ", "Ⅴ", "Ⅵ", "Ⅶ", "Ⅷ", "Ⅸ", "Ⅹ" }
--awful.util.tagnames = { "⠐", "⠡", "⠲", "⠵", "⠻", "⠿" }
...
globalkeys = my_table.join(
    ...
    awful.key({ modkey }, "F6", function () awful.util.spawn( "playerctl --all-players previous" ) end,
        {description = "Previous", group = "3- Player Control"}),
    awful.key({ modkey }, "F7", function () awful.util.spawn( "playerctl --all-players play-pause" ) end,
        {description = "Play / Pause", group = "3- Player Control"}),
    awful.key({ modkey }, "F8", function () awful.util.spawn( "playerctl --all-players next" ) end,
        {description = "Next", group = "3- Player Control"}),

    -- super + ... Programs
    awful.key({ modkey }, "f", function () awful.util.spawn( filemanager ) end,
        {description = "Thunar (File Manager)" , group = "1- Programs" }),
    awful.key({ modkey }, "r", function () awful.spawn.with_shell( "$HOME/.config/rofi/launchers/type-3/launcher.sh" ) end,
    ...
    awful.key({ modkey }, "e", function () awful.util.spawn( editorgui ) end,
        {description = "Visual Studio Code", group = "1- Programs"}),
    ...
    -- ctrl+alt +  ...
    awful.key({ modkey1, altkey   }, "t", function() awful.util.spawn( "arcolinux-tweak-tool" ) end,
        {description = "ArcoLinux Tweak Tool", group = "1- Programs"}),
    awful.key({ modkey1, altkey   }, "i", function() awful.util.spawn("nitrogen") end,
        {description = "nitrogen", group = "1- Programs"}),
    awful.key({ modkey1, altkey   }, "o", function() awful.spawn.with_shell("$HOME/.config/awesome/scripts/picom-toggle.sh") end,
        {description = "Picom Toggle", group = "2- Hotkeys"}),
    awful.key({ modkey1, altkey   }, "p", function() awful.util.spawn( "pamac-manager" ) end,
        {description = "Pamac Manager", group = "1- Programs"}),
   ...
   -- awful.key({ modkey }, "Right", function () lain.util.tag_view_nonempty(1) end,
    ...
    -- Default client focus
        ...
        awful.key({ modkey1, modkey }, "Down",
            ...
            {description = "Focus Down", group = "0- Window Manager"}),
        awful.key({ modkey1, modkey }, "Up",
            function()
                awful.client.focus.global_bydirection("up")
                if client.focus then client.focus:raise() end
            end,
            {description = "Focus Up", group = "0- Window Manager"}),
        awful.key({ modkey1, modkey }, "Left",
            function()
                awful.client.focus.global_bydirection("left")
                if client.focus then client.focus:raise() end
            end,
            {description = "Focus Left", group = "0- Window Manager"}),
        awful.key({ modkey1, modkey }, "Right",
            function()
                awful.client.focus.global_bydirection("right")
                if client.focus then client.focus:raise() end
            end,
            {description = "Focus Right", group = "0- Window Manager"}),


    -- Layout manipulation
    awful.key({ modkey, altkey   }, "Right", function () awful.client.swap.byidx(  1)    end,
              {description = "Swap With Next Client by Index", group = "0- Window Manager"}),
    ...
    awful.key({ modkey }, "w", function ()
        ...
        {description = "Toggle Wibox", group = "0- Window Manager"}),

 -- Show/Hide Systray
    awful.key({ modkey }, "-", function ()
    awful.screen.focused().systray.visible = not awful.screen.focused().systray.visible
    end, {description = "Toggle Systray Visibility", group = "0- Window Manager"}),

    -- On the fly useless gaps change
    awful.key({ altkey, "Control" }, ".", function () lain.util.useless_gaps_resize(1) end,
              {description = "Increment Useless Gaps", group = "0- Window Manager"}),
    awful.key({ altkey, "Control" }, ",", function () lain.util.useless_gaps_resize(-1) end,
              {description = "Decrement Useless Gaps", group = "0- Window Manager"}),

    -- Standard program
    awful.key({ modkey, }, "Return", function () awful.spawn(terminal) end,
              {description = "Alacritty", group = "1- Programs"}),
    awful.key({ modkey, "Shift" }, "r", awesome.restart,
              {description = "Reload Awesome", group = "0- Window Manager"}),
    ...
    awful.key({ modkey, altkey }, "c",
              ...
              {description = "Restore Minimized", group = "0- Window Manager"}),
...
clientkeys = my_table.join(
    ...
    {description = "Toggle Fullscreen", group = "0- Window Manager"}),
    awful.key({ modkey, }, "q", function (c) c:kill()                         end,
              {description = "Close", group = "2- Hotkeys"}),
    awful.key({ modkey, "Control" }, "Return", function (c) c:swap(awful.client.getmaster()) end,
              {description = "Move to Master", group = "0- Window Manager"}),
    awful.key({ modkey, "Shift"   }, "Left",   function (c) c:move_to_screen()               end,
              {description = "Move To Screen", group = "0- Window Manager"}),
    awful.key({ modkey, "Shift"   }, "Right",  function (c) c:move_to_screen()               end,
              {description = "Move To Screen", group = "0- Window Manager"}),    
    --awful.key({ modkey,           }, "t",      function (c) c.ontop = not c.ontop            end,
              --{description = "toggle keep on top", group = "0- Window Manager"}),
    awful.key({ modkey,           }, "c",
        function (c)
        ...
        {description = "Minimize", group = "0- Window Manager"}),
    awful.key({ modkey,           }, "m",
        function (c)
            c.maximized = not c.maximized
            c:raise()
        end ,
        {description = "Maximize", group = "0- Window Manager"})
)
...
for i = 1, 10 do
    ...
    if i == 1 or i == 9 then
        descr_view = {description = "View Tag #", group = "5- Tag"}
        descr_toggle = {description = "Toggle Tag #", group = "5- Tag"}
        descr_move = {description = "Move Focused Client to Tag #", group = "5- Tag"}
        descr_toggle_focus = {description = "Toggle Focused Client on Tag #", group = "5- Tag"}
    end
...
awful.spawn.with_shell("picom --experimental-backends --config $HOME/.config/awesome/picom.conf")
