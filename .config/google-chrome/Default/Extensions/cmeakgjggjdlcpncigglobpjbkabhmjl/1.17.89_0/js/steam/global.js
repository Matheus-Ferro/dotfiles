...
function formatNumberV2(totalPrice) {
  const currencyCode = +(typeof (g_rgWalletInfo) != 'undefined' && g_rgWalletInfo['wallet_currency'] ? g_rgWalletInfo['wallet_currency'] : 1)
  return v_currencyformat(totalPrice, GetCurrencyCode(currencyCode));
...
function getPriceAsIntV2(priceStr) {
  if (!priceStr) return 0;
...
function GetAvatarURLFromHash(hash, size) {
  var strURL = 'https://cdn.akamai.steamstatic.com/steamcommunity/public/images/avatars/' + hash.substring(0, 2) + '/' + hash;
...
function CEmoticonPopup($EmoticonButton, $Textarea) {
  ...
  var _this = this;
  this.m_$EmoticonButton.one('mouseenter', function () {
    _this.LoadEmoticons();
  });
  this.m_$EmoticonButton.click(function () {
    _this.LoadEmoticons();
    CEmoticonPopup.sm_deferEmoticonsLoaded.done(function () {
      _this.OnButtonClick();
    })
  });
  this.m_fnOnDocumentClick = function () {
    _this.DismissPopup();
  };
}
...
CEmoticonPopup.prototype.LoadEmoticons = function () {
  ...
  $J.get('https://steamcommunity.com/actions/EmoticonList')
    ...
    CEmoticonPopup.sm_deferEmoticonsLoaded.resolve()
  });
};
...
CEmoticonPopup.prototype.OnButtonClick = function () {
  if (this.m_bVisible) {
    ...
    window.setTimeout(function () {
      $J(document).one('click.EmoticonPopup', _this.m_fnOnDocumentClick)
    }, 0);
  }
};
...
function ShareOnSteam() {
  ...
  new Ajax.Updater("ShareOnSteamDialogContents", gShareRequestURL, {
    evalScripts: true, onLoaded: function () {
      ShowWithFade($('ShareOnSteamDialogContents'));
    }
  });
