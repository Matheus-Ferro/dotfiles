  ...
  conts.each(function (idx, el) {
    ...
    cont.find('.trade_item').each(function (i) {
      ...
      if (rgI) {
        ...
        {
          var price = parseInt(parseFloat(getPriceAsInt(rgI.lowestPrice)) * 100);

          // var publisherFee = typeof rgI.market_fee != 'undefined' ? rgI.market_fee : 0.1;
          var publisherFee = window.PriceUtils.publisherFee(rgI, g_rgWalletInfo, 0.1);

          var feeInfo = CalculateFeeAmount(price, publisherFee);
          totalNoTax += (price - feeInfo.fees) / 100;

          total += parseFloat(getPriceAsInt(rgI.lowestPrice));
        }

        if (rgI.gemPrices != 'undefined' && rgI.gemPrices != null) {
          $J.each(rgI.gemPrices, function (k, v) {
            gemprices += parseFloat(getPriceAsInt(v.lowestPrice));
          });
        }
      }
    ...
    var str = '' + formatNumberV2(total) + ' (' + formatNumberV2(totalNoTax) + ')';
    if (gemprices > 0) {
      str += ' - Gems total price: ' + formatNumberV2(gemprices);
    }
...
var GetAllPrice = function (parent) {
  parent.find('.trade_item').each(function () {
    if (this.inited == null) {
      PrebuildData(this, $J(this).attr('data-economy-item'));
    }
  });
};
