...
var removeListing = async function ($parent) {
  const cks = $parent.find('.market_listing_row .select-checkbox:visible:checked').toArray();

  const cancelBuyOrder = (item) => {
    let promise = new Promise((resolve, reject) => {
      const listingIdRow = $J(item).parents('.market_recent_listing_row').attr('id');
      let url = null;
      let params = {};
      if (listingIdRow.indexOf('mylisting') == 0) {

        const listingId = listingIdRow.substring(10);
        $J(`#${listingIdRow} .market_listing_edit_buttons.actual_content a`).html('Removing...');
        url = `//steamcommunity.com/market/removelisting/${listingId}`;
        params = {sessionid: g_sessionID};

      } else if (listingIdRow.indexOf('mybuyorder') == 0) {

        const buyOrderId = listingIdRow.substring(11);
        $J(`#${listingIdRow} .market_listing_edit_buttons.actual_content a`).html('Cancelling...');
  ...
  for (const i in cks) {
    const item = cks[i];
    const {err} = await cancelBuyOrder(item);
    if (!err) {
...
function linkInventory() {
  $J('.market_listing_row.market_recent_listing_row').each((ind, elm) => {
    const html = $J(elm).find('.market_listing_item_name').html();
    const gameName = $J(elm).find('.market_listing_game_name').text();

    const hashName = assetList.get($J(elm).find('.market_listing_item_name').text());
    if (hashName && apps[gameName] !== undefined) {
      const marketLink = `${window.location.protocol}//steamcommunity.com/market/listings/${apps[gameName]}/${encodeURIComponent(hashName)}`;
      $J(elm).find('.market_listing_item_name').html(`<a href="${marketLink}" target="_blank">${html}</a>`);
    }
  })
}
...
function marketMyHistoryRequestOnSuccessAfter(url, data, transport) {
  ...
  for (const appId in assets) {
    const items = assets[appId][1] || assets[appId][2];
