    ...
    if (rgItem.description.lowestPrice) {
      ...
      new Ajax.Request('https://steamcommunity.com/market/priceoverview/', {
        ...
        parameters: {
          ...
          currency: currencyId || (typeof (g_rgWalletInfo) != 'undefined' ? g_rgWalletInfo['wallet_currency'] : 1),
          appid: rgItem.appid,
...
const QuickSellForInventoryItem = (({
  ...
  return {
    addButton(item, container) {
      ...
      container.appendChild(createSellItemButton({...item}, sellingPrice, data));
    },
...
var SortItem = function (asc, cat = 'price') {
  ...
  g_ActiveInventory.m_bFullyLoaded = false;
...
const QuickSellAllFeature = {
  ...
  init() {
    $J('body').on('click', '#bt_quick_sell', () => this.run());
  },
...
var ModifySellingFunctions = function () {
  ...
  $J('#market_sell_dialog_accept_ssa_label').after('<div id="div_multi" style="display:none">' +
    ...
    '<p id="adjust-error-message" title="' + SIHLang.adjusterrormessagetooltip + '">'
    + SIHLang.adjusterrormessage
    + '</p>' +
    '</div>');
  ...
  $J('body').on('click', '.queue-item-remove', function () {
    ...
    if (Array.isArray(rgItemOrg.groupInventories) && rgItemOrg.groupInventories.length > 0) {
      ...
      loadNumberSelectedInventory(rgItemOrg);
    }
  ...
  setTimeout(function () {
    ...
    $J('body').on('click', '.floatbutton', function () {
      ...
      const updateFloatValue = function (data) {
        if (Array.isArray(CSGO_ORIGINS) && CSGO_ORIGINS.length) {
          ...
          if (!$curItem.find('.float-value').length) {
            $curItem.append(`<div class="float-value"><strong>${data.iteminfo.floatvalue.toFixed(4)}</strong></div>`);
          }
        }
      };
            ...
            <div id="sih-inventory-panel">
                <div class="row">
                    <div id="optionalPanel">
                     ...
                     <div id="switchGroupInventory">
                        <span class="title">Group inventory</span>
                        <label class="switch">
                            <input id="switcherGroupInventory" type="checkbox">
                            <span class="slider round"></span>
                        </label>
                    </div>
...
const GetFloatValues = () => {
  $J('div.item.app730').each((idx, elem) => {
    if ($J(elem).find('.group_inventory_flag').length === 0) {
